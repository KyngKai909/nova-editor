...
import { connectGithubOAuth } from "@/lib/githubOAuth";
...
export default function ConnectModal({ onClose }: { onClose: () => void }) {
  ...
  const oauthAvailable = isSupabaseConfigured();
  ...
  const startOAuth = async () => {
    setOauthError(null);
    setOauthBusy(true);
    const { error } = await connectGithubOAuth();
    if (error) {
      setOauthError(error);
      setOauthBusy(false);
    }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-line-2 bg-surface shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 p-5">
          {/* OAuth (live when Supabase auth is configured) */}
          <div className="group relative">
            <button
              onClick={oauthAvailable ? startOAuth : undefined}
              disabled={!oauthAvailable || oauthBusy}
              className={`flex w-full items-center justify-center gap-2 rounded-lg py-3 text-[14px] font-medium transition-colors ${
                oauthAvailable
                  ? "bg-ink text-bg hover:bg-white"
                  : "cursor-not-allowed border border-line bg-bg/50 text-ink-3"
              }`}
            >
              {oauthBusy ? (
          ...
          {oauthError && <p className="text-[12px] text-red-400">{oauthError}</p>}
