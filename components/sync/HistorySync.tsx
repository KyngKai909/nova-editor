...
export default function HistorySync() {
  ...
  const active = !!supabase && !!projectId && !!cloudOwner && !!myId && (ownerId ? role === "owner" || role === "editor" : isPaid);

  const lastPushed = useRef<string>("");

  // Pull MY history on open — the cloud is the cross-device source of truth.
  useEffect(() => {
    if (!active) return;
    let cancelled = false;
    pullCloudHistory(cloudOwner!, projectId!, myId!).then((h) => {
  ...
  // Push MY history on change (debounced). Skip the no-op echo right after a pull.
  useEffect(() => {
    if (!active) return;
    ...
    const t = setTimeout(() => {
      ...
      pushCloudHistory(cloudOwner!, projectId!, myId!, { past, future });
    }, 3000);
    return () => clearTimeout(t);
  }, [active, past, future, cloudOwner, projectId, myId]);
